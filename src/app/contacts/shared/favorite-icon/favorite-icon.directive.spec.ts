...
describe('Directive: FavoriteIconDirective', () => {
  ...
  describe('when favorite icon is set to true', () => {
    ...
    it('should display a solid gold star after the page loads', () => {
      ...
      expect(
        doClassesMatch(starElement.classList, expectedSolidStarList)
      ).toBeTruthy();
    });
    ...
    it('should display a black outline of a star after the user cicks on the start', () => {
      const event = new Event('click');
      starElement.dispatchEvent(event);
      expect(starElement.style.color).toBe('black');
      expect(
        doClassesMatch(starElement.classList, expectedOutlineStarList)
      ).toBeTruthy();
    });
  });
});
